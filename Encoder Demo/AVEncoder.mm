...
@interface AVEncoder ()<H264HwEncoderImplDelegate>
...
-(void) initializeEncoder:(int)width andheight: (int)height
{
...
//    _writer = [VideoEncoder encoderForPath:[self makeFilename] Height:height andWidth:width];
    [self initializeEncoder: width andheight:height];
}
...
- (void) sendForEncoding:(CMSampleBufferRef) sampleBuffer
{
    @synchronized(self)
    {
        if (_needParams)
